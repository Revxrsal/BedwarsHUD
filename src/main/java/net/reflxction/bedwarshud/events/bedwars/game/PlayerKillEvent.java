...
/**
 * Fired every time the player receives a kill. This isn't fired when it's a final kill.
 * If it's a final kill, {@link PlayerFinalKillEvent} is fired.
 */
public class PlayerKillEvent extends Event {
    ...
    public void onClientChatReceived(ClientChatReceivedEvent event) {
        ...
        if (text.startsWith(getName())) return; // Means the player died
        for (String kill : messages.getKillMessages()) {
            if (text.contains(kill + " " + getName()) && !text.endsWith("FINAL KILL!")) {
                MinecraftForge.EVENT_BUS.post(new PlayerKillEvent());
                break;
            }
        }
    }
